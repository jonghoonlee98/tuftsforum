...
function saveValuesToCookie(token, res) {
  ...
  console.log(user);
  res.cookie('user_email', user.email, {maxAge: 3600000, httpOnly: true});
  res.cookie('domain', (user.email).split('@')[1], {maxAge: 3600000, httpOnly: true});
...
function clearCookies(res) {
  // Clear cookies
  res.clearCookie('user_email', {maxAge: 3600000, httpOnly: true});
  res.clearCookie('domain', {maxAge: 3600000, httpOnly: true});
